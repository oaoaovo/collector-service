...
std::string MockDriverServer::handleQuery(const std::string& message) const {
    ...
    std::string response = "{\"cid\":\"" + escapeJson(cid) + "\",\"machineConnected\":true,\"data\":{";
    bool first = true;
    for (const auto& path : paths) {
        ...
        response += "\"item_path\":\"/DV:mock/CP:mock/DT:" + escapeJson(name) + "\",";
        response += "\"item_description\":\"" + escapeJson(descriptionForPath(path)) + "\",";
        response += "\"item_name\":\"" + escapeJson(name) + "\"";
        response += "}";
    }
    response += "}}";
    return response;
